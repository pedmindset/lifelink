            ...
            @switch($s['fieldType'])
               ...
               @case(2)
                  <div class="mt-3">
                     <label for="{{ $s['model'] }}" class="block text-sm capitalize font-bold text-gray-900">{{ $s['fieldName'] }}</label>
                     <div class="mt-1">
                        <textarea name="{{ $s['model'] }}" wire:change.prevent="getFieldValue($event.target.value,'{{ $s['fieldName'] }}')" rows="4" {{ $s['rules'] ? 'required' : '' }} id="{{ $s['model'] }}" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md"></textarea>
                     </div>
                  </div>
                  @break
                  @case(3)
                  <div class="mt-3">
                     <label for="{{ $s['model'] }}" class="block text-sm capitalize font-bold text-gray-900">{{ $s['fieldName'] }}</label>
                     <select name="{{ $s['model'] }}" wire:change.prevent="getFieldValue($event.target.value,'{{ $s['fieldName'] }}')" id="{{ $s['model'] }}" {{ $s['rules'] ? 'required' : '' }} class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                     ...
                     <fieldset class="mt-4">
                        ...
                        <div class="space-y-4 sm:flex sm:items-center sm:space-y-0 sm:space-x-10">
                           ...
                           <div class="flex items-center">
                              <input id="{{ $op[$loop->index] }}" name="{{ $s['model'] }}" wire:change.prevent="getFieldValue($event.target.value,'{{ $s['fieldName'] }}')" value="{{ $op }}" {{ $s['rules'] ? 'required' : '' }} type="radio" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300">
                     ...
                     <fieldset class="space-y-5">
                        ...
                        <div class="relative flex items-start">
                           <div class="flex items-center h-5">
                              <input id="{{ $op[$loop->index] }}" name="{{ $s['model'] }}" {{ $s['rules'] ? 'required' : '' }} wire:change.prevent="getFieldValue($event.target.value,'{{ $s['fieldName'] }}')" value="{{ $op }}" {{ $s['rules'] ? 'required' : '' }} aria-describedby="comments-description" name="comments" type="checkbox" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                           </div>
