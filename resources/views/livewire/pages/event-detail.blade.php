<div x-data="{ showApply: @entangle('applyMode') }">
    ...
    <div class="p-3 sm:px-16 sm:py-5">
        ...
        <div class="overflow-hidden">
            <img class="object-cover object-center w-full h-64 rounded-sm lg:h-80"
                src="{{ $event->image_url != '' ? $event->image_url : asset('img/back_con.jpg') }}"
                alt="conference image">
            <div class="max-w-5xl mx-auto">
                <p
                    class="flex items-center justify-center pt-12 font-sans text-2xl font-bold leading-6 tracking-wide text-green-600 uppercase">
                    {{ $event->name }}</p>
                <p class="py-4 text-base leading-9 tracking-wide text-gray-600">{{ $event->description ?? '' }}</p>
                <div class="flex space-x-8">
                    <p class="text-sm font-bold text-gray-600">From <span
                            class="px-3 py-1 rounded bg-cyan-200 text-cyan-600">{{ date("F jS, Y", strtotime($event->start_date)) }}</span>
                    </p>
                    <p class="text-sm font-bold text-gray-600">To <span
                            class="px-3 py-1 text-red-600 bg-red-200 rounded">{{ date("F jS, Y", strtotime($event->end_date)) }}</span>
                    </p>
                </div>
                ...
                <div class="pt-20 mb-12">
                    ...
                    <div class="overflow-hidden bg-white">
                        <ul role="list" class="divide-y divide-gray-200">
                                    ...
                                    <div class="px-4 py-4 sm:px-6">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <div class="flex flex-shrink-0 ml-2">
                                                <x-button type="button" wire:click="applyForm({{$app->id }})"
                                                    class="inline-flex text-xs font-bold leading-5 text-green-800 bg-green-100 rounded-md hover:bg-green-400 hover:text-white hover:shadow-inner">
                                                    Apply
                                                </x-button>
                                            </div>
                                        </div>
                            ...
                            @empty
                            <li class="p-3 font-sans text-sm font-bold text-green-600 bg-green-100 rounded-sm">No Forms
                                to apply</li>
                            @endforelse
                        </ul>
                    </div>
                            ...
                            <div>
                                ...
                                <dd class="mt-3 text-sm text-gray-400">
                                    <ul role="list" class="mt-4 space-y-4">
                                        ...
                                        <li>International Delegates:
                                            ${{ number_format($event->fee->international_amount, 2) }} US DOLLARS</li>
                                        @endif

                                        @if (isset($event->fee->early_bird_amount))
                                        <li>Early bird registration -
                                            ¢{{ number_format($event->fee->early_bird_amount, 2) }} before
                                            {{ date("F jS, Y", strtotime($event->fee->early_bird_date)) }}</li>
                                        @endif
                                        <li>Regular registration - ¢{{ number_format($event->fee->standard_amount, 2) }}
                                            before {{ date("F jS, Y", strtotime($event->fee->regular_date)) }}</li>
                                        @if (isset($event->fee->late_amount))
                                        <li>Late registration ¢{{ number_format($event->fee->late_amount, 2) }} after
                                            {{ date("F jS, Y", strtotime($event->fee->late_date)) }}</li>
                                        @endif
                                        @else
                ...
                <div class="p-8">
                    <div class="border-2 border-gray-500 rounded">
                        <iframe
                            ...
                            height="450" frameborder="0" width="100%" style="border:0" loading="lazy"></iframe>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="bg-gray-100 z-10" style="min-height: 320px;">
        ...
        <div x-data="{ show: @entangle('access_denied') }" aria-live="assertive"
            class="fixed inset-0 flex items-end px-4 py-6 pointer-events-none sm:p-6 sm:items-start mt-16 z-10">
                    ...
                    <div class="p-4">
                        <div class="flex items-start">
                            <div class="flex-shrink-0">
                                <svg class="h-6 w-6 text-red-600" x-description="Heroicon name: outline/check-circle"
...
<script>
    window.addEventListener('alertMessage', ({
        detail: {
            type,
            message
        }
