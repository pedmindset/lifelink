<section>
   <div class="pb-5 space-y-3 flex-1 flex-col flex">
      @foreach ($schema as $index => $s)
      @switch($s->fieldType)
         @case(1)
         <div class="mt-3">
            <label for="{{ $s->model }}" class="block text-sm font-bold text-gray-900">{{ $s->fieldName }}</label>
            <div class="mt-1">
               <input wire:change.prevent="getFieldValue($event.target.value,'{{ $s->fieldName }}')" {{ $s->rules ? 'required' : '' }} {{ $attributes->merge(['type'=>'text','id'=>$s->model,'placeholder'=> $s->placeholder, 'class'=>'block w-full shadow-sm sm:text-sm focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md' ]) }}>
            </div>
         </div>
         @break
         @case(2)
